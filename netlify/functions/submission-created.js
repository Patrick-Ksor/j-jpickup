...
exports.handler = async (event) => {
  try {
    ...
    const address   = d.address           || '(not provided)';
    const junkDesc  = d['junk-desc']      || '(not provided)';
    const service   = d.service           || '';
    const loadSize  = d['load-size']      || '';
    const preferred = d['preferred-date'] || '';
    const formName  = payload.form_name;
    const submittedAt = new Date(payload.created_at).toLocaleString('en-US', {
    ...
    if (email && email !== '(not provided)') {
      await transporter.sendMail({
        ...
        html:    customerTemplate({ name, phone, address, junkDesc }),
      });
    }
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,.10);">
        ...
        <tr>
          <td style="background:#2ecc71;padding:10px 32px;text-align:center;">
            <p style="margin:0;color:#fff;font-size:13px;font-weight:600;">&#9889; Reply within 1 hour to beat competitors &mdash; hit Reply to go straight to the customer.</p>
          </td>
        </tr>
            ...
            <table cellpadding="0" cellspacing="0">
              <tr>
                <td style="padding-right:12px;">
                  <a href="tel:${phone}" style="display:inline-block;background:#2ecc71;color:#fff;text-decoration:none;padding:12px 24px;border-radius:6px;font-weight:700;font-size:14px;">&#128222; Call Now</a>
                </td>
                <td>
                  <a href="mailto:${email}" style="display:inline-block;background:#1e73be;color:#fff;text-decoration:none;padding:12px 24px;border-radius:6px;font-weight:700;font-size:14px;">&#9993;&#65039; Reply by Email</a>
                </td>
              </tr>
            </table>
          </td>
...
const customerTemplate = ({ name, phone, address, junkDesc }) => `
<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>
<body style="margin:0;padding:0;background:#f0f2f5;font-family:Inter,Arial,sans-serif;">
      ...
      <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#fff;border-radius:10px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,.10);">
        ...
        <tr>
          <td style="background:#1e73be;padding:32px;text-align:center;">
            ...
            <p style="margin:0;color:rgba(255,255,255,.85);font-size:14px;">J&amp;J Pickup &middot; Greensboro, NC</p>
          </td>
        </tr>
        ...
        <tr>
          <td style="padding:32px;">
            <p style="margin:0 0 16px;font-size:16px;color:#1a1a2e;line-height:1.6;">
              Thanks for reaching out! We've received your quote request and will get back to you
              <strong style="color:#1e73be;">within 1 hour</strong> during business hours.
            </p>

            <!-- Summary box -->
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#f5f7fa;border-radius:8px;border:1px solid #e8ecf0;margin:20px 0;">
              <tr><td style="padding:16px 20px;">
                <p style="margin:0 0 10px;font-size:13px;font-weight:700;color:#555;text-transform:uppercase;letter-spacing:.06em;">Your Submission Summary</p>
                <p style="margin:4px 0;font-size:14px;color:#333;"><strong>Pickup Address:</strong> ${address}</p>
                <p style="margin:4px 0;font-size:14px;color:#333;"><strong>Items:</strong> ${junkDesc}</p>
              </td></tr>
            </table>
            ...
            <p style="margin:0 0 24px;font-size:14px;color:#555;line-height:1.6;">
              Need to reach us sooner? Give us a call &mdash; we're ready to help:
            </p>

            <table cellpadding="0" cellspacing="0">
              <tr>
                <td>
                  <a href="tel:${phone}" style="display:inline-block;background:#2ecc71;color:#fff;text-decoration:none;padding:14px 28px;border-radius:6px;font-weight:700;font-size:15px;">&#128222; Call J&amp;J Pickup</a>
                </td>
              </tr>
            </table>
          </td>
        ...
        <tr>
          <td style="background:#f5f7fa;padding:20px 32px;text-align:center;border-top:1px solid #e8ecf0;">
            ...
            <p style="margin:0;font-size:12px;color:#aaa;">You're receiving this because you submitted a quote request on jjpickup.com.</p>
          </td>
        </tr>
